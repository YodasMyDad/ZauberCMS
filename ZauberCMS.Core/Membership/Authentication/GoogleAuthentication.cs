...
public class GoogleAuthentication : IExternalAuthenticationProvider
{
    ...
    {
        var googleId = configuration.GetValue<string>("Zauber:Identity:ExternalProviders:Google:ClientId");
        if (!googleId.IsNullOrWhiteSpace())
        {
            ...
            authenticationBuilder.AddGoogle(options =>
            {
                ...
                options.ClientSecret = configuration.GetValue<string>("Zauber:Identity:ExternalProviders:Google:ClientSecret") ?? "";
            });
        }
    }
}