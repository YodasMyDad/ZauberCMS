...
public class DefaultAssemblyProvider
{
    ...
    public DefaultAssemblyProvider(IServiceProvider serviceProvider, ZauberSettings settings)
    {
        ...
        this.IsCandidateAssembly = assembly =>
        {
            ...
            return !_exclusionList.Any(exclusion =>
                assemblyName.StartsWith(exclusion, StringComparison.OrdinalIgnoreCase));
        };
        ...
        this.IsCandidateCompilationLibrary = library =>
        {
            ...
            return !_exclusionList.Any(exclusion =>
                libraryName.StartsWith(exclusion, StringComparison.OrdinalIgnoreCase));
        };
    }
    ...
    private void GetAssembliesFromDependencyContext(List<Assembly?> assemblies)
    {
        ...
        // Use CompileLibraries as user should have set PreserveCompilationContext if single file publishing
        if (DependencyContext.Default?.CompileLibraries != null)
        {
            ...
            {
                ProcessLibrary(compilationLibrary, assemblies);
            }
        }
        // Fallback to RuntimeLibraries
        else if (DependencyContext.Default?.RuntimeLibraries != null)
        {
    ...
    private void ProcessLibrary(Library compilationLibrary, List<Assembly?> assemblies)
    {
        if (IsCandidateCompilationLibrary(compilationLibrary))
        {
            ...
            try
            {
                        ...
                        string.Equals(a?.FullName, assembly.FullName, StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    logger.LogInformation("Assembly '{FullName}' is discovered and loaded", assembly.FullName);
                }
            }
        ...
        if (!string.IsNullOrEmpty(path) && Directory.Exists(path))
        {
            ...
            foreach (var extensionPath in Directory.EnumerateFiles(path, "*.dll"))
            {
                ...
                try
                {
                    ...
                    if (this.IsCandidateAssembly(assembly) && !assemblies.Any(a =>
                            string.Equals(a?.FullName, assembly.FullName, StringComparison.OrdinalIgnoreCase)))
                    {
                        assemblies.Add(assembly);
